...
type SecRule struct {
	...
	// Determines whether the string identified in a variable object will
	// be transformed and the metadata that will be assigned to malicious traffic.
	...
	*/
	...
	/*
		...
		Valid values are:
			...
			URLDECODE: Indicates that the source value should be URL decoded. This transformation
						is useful when the source value has been URL encoded twice.
			REMOVENULLS: Indicates that null values should be removed from the source value.

		Note: A criterion is satisfied if the source value or any of the modified string values
		meet the conditions defined by the operator object.
	*/
...
type ChainedRule struct {
	...
	// Determines whether the string value derived from the request element identified
	// in a variable object will be transformed and the metadata that will be used to identify malicious traffic.
	Action Action `json:"action"`

	// Indicates the comparison that will be performed on the string value(s) derived
	// from the request element(s) defined within the variable array.
	Operator Operator `json:"operator"`
...
type Variable struct {
	...
	/*
		...
		Note: If a request element consists of one or more key-value pairs, then you
		may identify a key via a match object.
		If is_count has been disabled, then you may identify a specific value via the operator object.
	*/
	...
	/*
		Determines whether a comparison will be performed between the operator object and
		a string value or the number of matches found.

		Valid values are:

		true: A counter will increment whenever the request element defined by this variable object
		is found. The operator object will perform a comparison against this number.
		** Note: If you enable is_count, then you must also set the type property to EQ.**

		false: The operator object will perform a comparison against the string value derived from
		the request element defined by this variable object.

	*/
	IsCount bool `json:"is_count,omitempty"`
}

// Operator describes the comparison that will be performed on the request element(s) defined
// within a variable object using its properties:
type Operator struct {

	/*
		Indicates whether a condition will be satisfied when the value derived from
		the request element defined within a variable object matches or does not match the value property.
	...
	*/
	...
	/*
		Indicates how the system will interpret the comparison between the value property
		and the value derived from the request element defined within a variable object.
	...
	*/
	...
	/*
		Indicates a value that will be compared against the string or number value derived from
		the request element defined within a variable object.

		Note: If you are identifying traffic via a URL path (REQUEST_URI), then you should
		specify a URL path pattern that starts directly after the hostname. Exclude a protocol
		or a hostname when defining this property.
...
type Match struct {
	...
	*/
	...
	/*
		...
		If the type property is set to REQUEST_HEADERS and this property is set to User-Agent,
		then this match condition is restricted to the User-Agent request header.

		If the value property is omitted, then this match condition applies to all request headers.
	*/
	Value string `json:"value,omitempty"`
}
